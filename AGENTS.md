...
- **DuckDB-WASM is the data layer.** `src/hooks/useDuckDB.ts` owns the entire DB lifecycle: it lazily instantiates DuckDB-WASM (self-hosted bundles served from `public/duckdb-wasm/` via a Blob worker) and registers fetched/uploaded data files into DuckDB's virtual filesystem via `registerFileBuffer` (`loadRemoteFile` for URLs, `loadLocalCsvFile` for local uploads).
