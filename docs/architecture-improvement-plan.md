...
gantt
    ...
    Run 上書き保護 (409 Conflict)     : p3_2, after p3_1, 2d
    AWS CDK / 不要リソースの完全撤廃  : p3_3, after p3_2, 2d
...
- [ ] **Task 3.1: ディスク容量クォータ & 自動クリーンアップ**
  - [ ] アップロード時に空きディスク容量をチェックし、閾値未満なら 507 エラーを返却
  - [ ] 一定期間（例: 30日）経過した Run の大容量ファイル（動画等）を自動削除するメンテナンスタスクを作成
- [ ] **Task 3.2: Run データの改ざん防止**
  - [ ] 既存の `manifest.json` が存在する Run に対する上書きをデフォルト拒絶（`409 Conflict`）
- [ ] **Task 3.3: AWS CDK / 不要コードの整理**
  - [ ] DynamoDB スタックおよびオンプレミス用の IAM ユーザー定義を整理・廃止
