...
- **オンラインデータベースバックアップ (`backup.py`)**:
  - Python `sqlite3.Connection.backup()` を使用したアトミックなオンラインスナップショット生成。
  - バックアップ生成直後の `PRAGMA integrity_check;` 自動実行による破損防止。
  - gzip 圧縮および指定世代（デフォルト: 14日）を超過した古いバックアップの自動パージ。
- **環境変数設定**:
...
### 7.3 永続化ストレージマウント設計（オンプレミス本番運用）
本番環境において、大容量成果物およびデータベースをホスト上の永続ストレージに保存するための設定仕様です。

1. **ストレージ分離の原則**:
   - **大容量成果物 (`/data/runs`)**:
     - テスト結果メトリクス、UE ログ、キャプチャ動画、クラッシュダンプ等を格納。
     - ホストマシンの大容量 HDD / RAID / NAS マウントディレクトリ（NFS, SMB）を割り当てます。
   - **SQLite WAL データベース (`/data/db`)**:
     - `qa.db` を格納。
     - SQLite WAL モードの共有メモリ (`-shm`) およびファイルロック整合性を担保するため、**必ずホストの高速ローカル SSD** に配置します（ネットワークストレージ上への配置は禁止）。
2. **環境変数によるバインドマウント指定**:
   - `onprem/.env` にて以下の環境変数を設定することで、Docker Compose の名前付きボリュームからホストディレクトリのバインドマウントへシームレスに切り替わります（未指定時は Docker Named Volume `qa_data` / `qa_db` にフォールバック）。
     - `HOST_STORAGE_DATA_PATH`: ホスト側の成果物格納ディレクトリ絶対パス（例: `/mnt/storage/game-qa/runs`）
     - `HOST_STORAGE_DB_PATH`: ホスト側の SQLite DB 格納ディレクトリ絶対パス（例: `/var/lib/game-qa/db`）
3. **Nginx ゼロコピー配信とのパス同期**:
   - Nginx は `/data/runs/` を `sendfile` によりゼロコピー直配信するため、Backend と同じ成果物ディレクトリを読み取り専用（`:ro`）でマウントします。

### 7.4 データベースの安全なオンラインバックアップ (`backup.py` / `backup_db.sh`)
1. **WAL 整合性の担保**:
   - SQLite WAL モード稼働中に単純なファイルコピーを行うと、`-wal` / `-shm` ファイル間の不整合により DB が破損するリスクがあります。
   - `sqlite3.Connection.backup()` API を利用し、ロックフリーかつトランザクション整合性を保証したスナップショットを生成します。
2. **自動整合性検証 & 圧縮**:
   - バックアップ生成直後に `PRAGMA integrity_check;` を実行し、整合性が確認されたもののみを正式なバックアップとして確定。
   - gzip 圧縮によりディスクフットプリントを最小化（例: `.db.gz`）。
3. **世代管理 & 定期自動化**:
   - 指定保持期間（デフォルト 14 日）を超過した過去のバックアップファイルを自動パージ。
   - `onprem/scripts/backup_db.sh` を crontab または systemd timer から定期実行（推奨: 毎日深夜 02:00）。

### 7.5 コンテナ監視・ヘルスチェック・ログローテーション
1. **死活監視 & ヘルスチェック (`/api/health`)**:
   - Backend コンテナは内部 Python ワンライナーにより `/api/health` の応答を 10 秒間隔で検証。
   - Nginx は Backend が `service_healthy` に達してからリクエスト受付を開始。
   - DMZ ロードバランサーや監視ツールからの `/api/health` リクエストは、OAuth2 認証をバイパスして直接 Backend ヘルスチェックへ中継されるため、監視アラートの誤検知を防止。
2. **Docker ログローテーション**:
   - 全コンテナ（Nginx, OAuth2-Proxy, Backend）に `json-file` ロギングドライバ（`max-size: 50m`, `max-file: 5`）を設定し、コンテナログによるホストディスク枯渇を防止。
3. **本番自動デプロイスクリプト (`onprem/deploy.sh`)**:
   - 事前環境チェック（Docker、.env、OAuth設定、永続ストレージ権限、フロントエンドビルド）、コンテナビルド・起動、ヘルスチェック検証をワンストップで実行。
